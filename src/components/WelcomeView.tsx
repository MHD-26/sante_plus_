...
export default function WelcomeView({
    ...
    switch (role) {
      ...
      case UserRole.ACCUEIL:
        return {
          ...
          actions: [
            { label: "Planifier un Rendez-vous", view: "rendezvous", autoOpen: true },
  ...
  const services = [
    {
      ...
      desc: "Consultations courantes, diagnostics de précision et médecine préventive.",
      icon: Stethoscope,
    },
    {
      name: "Urgences 24h/7",
      desc: "Prise en charge réactive immédiate avec équipe médicale de garde permanente.",
      icon: HeartPulse,
    },
    {
      name: "Hospitalisation",
      desc: "Chambres modernes climatisées, équipées pour un suivi sécurisé continu.",
      icon: Bed,
    },
    {
      name: "Pédiatrie",
      desc: "Consultations pédiatriques, vaccinations et soins spécialisés pour les enfants.",
      icon: Baby,
    },
    {
      name: "Gynécologie",
      desc: "Suivi gynécologique complet, obstétrique et accompagnement à la maternité.",
      icon: Users,
    },
    {
      name: "Cardiologie",
      desc: "Dépistage, ECG, examens cardiovasculaires et traitements personnalisés.",
      icon: Heart,
    },
    {
      name: "Laboratoire d'Analyses",
      desc: "Analyses de biologie clinique, bilans sanguins et diagnostics rapides.",
      icon: FlaskConical,
    },
    {
      name: "Imagerie Médicale",
      desc: "Radiographie numérique, échographie générale et doppler de pointe.",
      icon: Activity,
    },
    {
      name: "Pharmacie Intégrée",
      desc: "Disponibilité immédiate des traitements et médicaments de qualité.",
      icon: Pill,
    },
  ];
  ...
  const patientRecord = patients.find((p) => {
    ...
    if (
      authenticatedUser.dossierNumber &&
      p.id.toLowerCase() === authenticatedUser.dossierNumber.toLowerCase()
    ) {
      return true;
    }
    ...
    if (
      ...
      p.email.toLowerCase() === authenticatedUser.email.toLowerCase()
    ) {
      return true;
    }
    ...
    if (
      ...
      p.phone.replace(/[^0-9]/g, "") === authenticatedUser.phone.replace(/[^0-9]/g, "")
    ) {
      return true;
    }
  ...
  const myAppointments = appointments.filter((app) => {
    return (
      app.patientId === activePatient.id ||
  ...
  const pendingConfirmations = appointments.filter(
    (app) => app.status === "En attente de confirmation"
  );
  ...
  const [compCategory, setCompCategory] = useState<
    "Accueil" | "Temps d'attente" | "Qualité des soins" | "Tarification" | "Autre"
  >("Qualité des soins");
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs md:text-sm text-emerald-100/90 font-medium max-w-xl">
                Bienvenue dans votre espace santé personnel sécurisé. Vous pouvez consulter vos
                dossiers, planifier vos visites et échanger avec nos équipes.
              </p>
            </div>
          </div>
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-xs">
            ...
            <div className="space-y-6">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">
                  Retrouvez ici vos comptes rendus cliniques et vos ordonnances médicales prescrites
                  à la Clinique Santé Plus.
                </p>
              </div>
              ...
              {activePatient.medicalHistory && activePatient.medicalHistory.length > 0 ? (
                    ...
                    <div key={hist.id} className="relative">
                      ...
                      <div className="bg-slate-50 hover:bg-slate-100/80 border border-slate-200/60 rounded-xl p-5 space-y-3.5 transition-all">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-200/40 pb-2">
                          <div className="flex items-center gap-2">
                            <span className="text-xs font-black text-slate-700 bg-white border border-slate-200 px-2 py-0.5 rounded-md font-mono">
                              {hist.id}
                            </span>
                ...
                <div className="text-center py-10 bg-slate-50 rounded-xl border border-dashed border-slate-200 p-6 space-y-2">
                  ...
                  <p className="text-xs font-bold text-slate-500">
                    Aucun historique de consultation enregistré
                  </p>
                  <p className="text-[11px] text-slate-400 max-w-sm mx-auto">
                    Si vous venez de créer votre compte, votre médecin l'alimentera lors de votre
                    prochaine consultation à la clinique.
                  </p>
                </div>
              )}
            </div>
                ...
                {myAppointments.length > 0 ? (
                  <div className="space-y-3">
                    {myAppointments.map((app) => (
                      <div
                        key={app.id}
                        className="bg-slate-50 border border-slate-200/60 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4"
                      >
                        <div className="space-y-1.5">
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] font-bold text-slate-400 font-mono bg-white border border-slate-200 px-1.5 py-0.5 rounded">
                              {app.id}
                            </span>
                          ...
                          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-slate-500 text-[11px] font-medium">
                            <span className="flex items-center gap-1">
                              <Calendar className="w-3.5 h-3.5 text-slate-400" /> {app.date} à{" "}
                              {app.time}
                            </span>
                            <span className="flex items-center gap-1">
                              <User className="w-3.5 h-3.5 text-slate-400" /> {app.doctorName}
                            </span>
                          </div>
                        </div>
              ...
              <div className="bg-slate-50 border border-slate-200/60 rounded-xl p-5 space-y-4">
                <div className="border-b border-slate-200/60 pb-2">
                  ...
                  <p className="text-[11px] text-slate-400 mt-0.5">
                    Planifiez une consultation externe
                  </p>
                </div>
                ...
                {rdvSubmitted ? (
                  <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-xl text-center space-y-2 animate-fade-in">
                    ...
                    <p className="text-[11px] text-emerald-600 leading-normal">
                      Votre demande a été enregistrée avec succès. Elle est actuellement en attente
                      de confirmation par le médecin choisi.
                    </p>
                  </div>
                ) : (
          ...
          {patientTab === "factures" && (
            <div className="space-y-6">
              <div>
                <h3 className="text-base font-extrabold text-slate-800 tracking-tight">
                  Vos Factures & Règlements
                </h3>
                <p className="text-xs text-slate-400 mt-1">
                  Retrouvez et imprimez l'historique de vos factures de soins et de pharmacie émis
                  par la clinique.
                </p>
              </div>
              ...
              {myInvoices.length > 0 ? (
                    ...
                    <tbody className="divide-y divide-slate-100 font-medium">
                      {myInvoices.map((inv) => (
                        <tr key={inv.id} className="hover:bg-slate-50/50">
                          <td className="py-3.5 px-2 font-mono font-black text-slate-800">
                            {inv.id}
                          </td>
                          ...
                          <td className="py-3.5 px-2 text-slate-500 font-bold">
                            {inv.paymentMethod}
                          </td>
                        </tr>
                      ))}
                ...
                <div className="text-center py-10 bg-slate-50 rounded-xl border border-dashed border-slate-200 p-6 space-y-2">
                  ...
                  <p className="text-xs font-bold text-slate-500">
                    Aucune facture enregistrée à votre nom
                  </p>
                  <p className="text-[11px] text-slate-400">
                    Les factures de consultations ou de médicaments apparaîtront dès qu'elles seront
                    validées par la caisse.
                  </p>
                </div>
              )}
            </div>
              ...
              <div className="space-y-4 lg:col-span-1">
                <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-5 space-y-3.5 text-emerald-900 leading-relaxed shadow-xs">
                  ...
                  <p className="text-xs font-medium">
                    La Clinique Santé Plus s'engage à fournir des soins d'excellence dans le respect
                    total de votre bien-être.
                  </p>
                  <p className="text-xs font-medium">
                    Vos critiques constructives et appréciations nous permettent de rehausser
                    continuellement la qualité de nos services d'accueil, d'administration et de
                    soins médicaux.
                  </p>
              ...
              <div className="lg:col-span-2 bg-slate-50 border border-slate-200/60 rounded-xl p-6 space-y-4">
                <div>
                  <h3 className="text-base font-extrabold text-slate-800">
                    Donner votre avis ou signaler une insatisfaction
                  </h3>
                  <p className="text-xs text-slate-400 mt-1">
                ...
                {compSubmitted ? (
                  <div className="p-6 bg-emerald-50 border border-emerald-200 rounded-xl text-center space-y-3 animate-fade-in">
                    ...
                    <p className="text-xs text-emerald-600 max-w-md mx-auto leading-relaxed">
                      Nous vous remercions sincèrement d'avoir pris le temps de nous écrire. Vos
                      remarques seront présentées en comité de direction pour améliorer notre
                      accueil et nos soins.
                    </p>
                  </div>
                ) : (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-slate-50 border border-slate-200/60 rounded-2xl p-6 md:p-8 font-sans">
          <div className="space-y-3">
            ...
            <p className="text-[11px] text-slate-500 leading-relaxed font-medium">
              Site Bingerville : Boulevard de la Nation, face au Jardin Botanique
              <br />
          ...
          <div className="space-y-3">
            ...
            <p className="text-[11px] text-slate-500 leading-relaxed font-medium">
              Urgences 24h/7 : <strong className="text-slate-700">+221 76 725 7479</strong>
              <br />
          ...
          <div className="space-y-3">
            ...
            <p className="text-[11px] text-slate-500 leading-relaxed font-medium">
              Consultations externes : Lun - Ven (08h - 18h), Sam (08h - 13h)
              <br />
    ...
    <div className="space-y-10 animate-fade-in py-4 max-w-5xl">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5 md:gap-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm md:text-base text-emerald-50/90 font-medium leading-relaxed max-w-3xl">
              Votre partenaire de confiance pour une prise en charge médicale personnalisée, moderne
              et sécurisée. Nous accompagnons chaque patient avec rigueur et humanité, du premier
              rendez-vous jusqu'au suivi thérapeutique complet.
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="flex flex-wrap gap-2.5 shrink-0">
            {roleMessage.actions.map((act, idx) => (
              <button
                key={idx}
                onClick={() => {
                  if (act.autoOpen && setAutoOpenAppointmentForm) {
          ...
          {pendingConfirmations.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {pendingConfirmations.map((app) => (
                <div
                  key={app.id}
                  className="border border-slate-200/80 hover:border-emerald-200/80 bg-slate-50/40 rounded-xl p-5 flex flex-col justify-between gap-4 transition-all"
                >
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[10px] font-bold text-slate-400">
                        Médecin :{" "}
                        <span className="text-emerald-700 font-extrabold">{app.doctorName}</span>
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 bg-white border border-slate-100 p-2.5 rounded-lg text-xs font-medium">
                      <div className="space-y-0.5">
                        ...
                        <span className="text-slate-700 flex items-center gap-1">
                          <Calendar className="w-3.5 h-3.5 text-slate-400" /> {app.date}
                        </span>
                      </div>
                      <div className="space-y-0.5">
                        <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block">
                        ...
                        <span className="text-slate-700 flex items-center gap-1">
                          <Clock className="w-3.5 h-3.5 text-slate-400" /> {app.time}
                        </span>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="text-xs font-bold text-slate-800 mt-0.5 leading-normal">
                        {app.reason}
                      </p>
                    </div>
                    ...
                    {app.notes && (
                      <div className="bg-amber-50/50 border border-amber-100/60 rounded-lg p-2.5 text-[11px] text-amber-900 leading-relaxed font-medium">
                        <span className="font-bold text-amber-950">Symptômes/Notes :</span>{" "}
                        {app.notes}
                      </div>
                    )}
                  </div>
            ...
            <div className="text-center py-10 bg-slate-50 rounded-xl border border-dashed border-slate-200 p-6 space-y-2">
              ...
              <p className="text-[11px] text-slate-400 max-w-sm mx-auto">
                Toutes les demandes de rendez-vous en ligne formulées par les patients ont été
                traitées. Félicitations !
              </p>
            </div>
          )}
        </div>
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Une infrastructure robuste et une expertise médicale dédiée au service de votre santé au
            quotidien.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
          {stats.map((stat, i) => {
            ...
            return (
              ...
              >
                <div
                  className={`p-2 rounded-lg ${stat.color} mb-3 group-hover:scale-110 transition-transform`}
                >
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Découvrez nos départements médicaux équipés de technologies modernes pour un diagnostic
            ...
            return (
              ...
              >
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-slate-500 leading-normal font-medium">{srv.desc}</p>
                </div>
              </div>
            );
          })}
          ...
          <div className="space-y-3.5 text-xs text-slate-600 font-medium">
            <div className="space-y-1 pl-1">
              ...
              <p className="leading-relaxed">
                Boulevard de la Nation, face au Jardin Botanique, Abidjan
              </p>
            </div>
        ...
        <div className="space-y-4 border-t md:border-t-0 border-slate-200/80 pt-5 md:pt-0">
          ...
          <div className="space-y-3 text-xs text-slate-600 font-medium">
            ...
            <div className="pl-1 space-y-1 pt-1 text-[11px] leading-relaxed">
              <p>
                <span className="font-bold text-slate-700">Consultations externes :</span>
              </p>
