...
export default function AppointmentsView({
  ...
  // Helper timestamps for spacing check
  const getAppointmentTimestamp = (dStr: string, tStr: string) => {
    return new Date(`${dStr}T${tStr}:00`).getTime();
  };

  // Verifier si un rdv chevauche un autre (doit etre espace d'au moins 30 minutes)
  const checkAppointmentConflict = (
    ...
    const newTimestamp = getAppointmentTimestamp(newDate, newTime);
    if (isNaN(newTimestamp)) return null;

    const minInterval = 30 * 60 * 1000; // 30 mins en ms

    for (const app of appointments) {
      if (excludeAppId && app.id === excludeAppId) continue;
      if (app.status === "Absent") continue; // Absent libere le créneau

      if (app.doctorName === doctor && app.date === newDate) {
        const appTimestamp = getAppointmentTimestamp(app.date, app.time);
        if (isNaN(appTimestamp)) continue;
  ...
  const openEditForm = (app: Appointment) => {
    ...
    setPatientSearchQuery(app.patientName);
  ...
  const handleSaveAppointment = (e: React.FormEvent) => {
    ...
    const conflict = checkAppointmentConflict(date, time, doctorName, editingApp?.id);
    if (conflict) {
      alert(
        `Conflit de planification : ${doctorName} a déjà un rendez-vous le ${date} à ${conflict.time} avec le patient ${conflict.patientName}.\n\nLes rendez-vous doivent être espacés d'au moins 30 minutes.`
      );
      return;
    }
                ...
                {!editingApp && (
                  <div className="space-y-2 bg-slate-50 p-3 rounded-xl border border-slate-200/60">
                    ...
                    {isNewPatient ? (
                      ...
                      <div className="space-y-1 pt-2 border-t border-slate-200/60 mt-2 relative">
                        <label className="font-bold text-slate-600 block">
                          Rechercher et Sélectionner un patient <span className="text-rose-500">*</span>
                        </label>
                        
                        {patientId ? (
                          // Patient selection confirmation card
                          <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3 flex justify-between items-center animate-fade-in">
                            <div className="space-y-1">
                              ...
                              <p className="text-[10px] text-slate-500 font-mono">
                                ID: {patientId} • Tél : {patients.find(p => p.id === patientId)?.phone}
                              </p>
                            </div>
                            <button
                              type="button"
                              ...
                              className="px-2.5 py-1 text-[10px] font-extrabold text-rose-700 bg-rose-50 hover:bg-rose-100 border border-rose-100 rounded-md cursor-pointer transition-all"
                            >
                              Changer
                            </button>
                          </div>
                          ...
                          <div className="space-y-1.5 relative">
                            <div className="relative z-30">
                              ...
                              <input
                                type="text"
                                placeholder="Tapez le prénom, nom ou téléphone du patient..."
                                ...
                                className="w-full pl-8 pr-8 py-2 border border-slate-200 rounded bg-white focus:outline-emerald-600 text-slate-800 text-xs font-semibold"
                              />
                              {patientSearchQuery && (
                                <button
                                  type="button"
                            ...
                            {showPatientSuggestions && (
                              <div className="absolute left-0 right-0 z-40 mt-1 max-h-48 overflow-y-auto bg-white border border-slate-200 rounded-lg shadow-lg divide-y divide-slate-100 text-xs">
                                {patients.filter(p => {
                                  ...
                                  patients.filter(p => {
                                    ...
                                    <button
                                      ...
                                      className="w-full text-left px-3 py-2 hover:bg-emerald-50 text-slate-700 hover:text-emerald-900 transition-colors flex justify-between items-center"
                                    >
                                      <div>
                                  ...
                                  <div className="p-3 text-center text-slate-400 text-xs italic">
                                    Aucun patient trouvé correspondant à "{patientSearchQuery}"
                                  </div>
                                )}
                              </div>
                            )}
                        ...
                        {/* Invisible click handler to close suggestions when clicking outside */}
                        {showPatientSuggestions && (
                          <div 
