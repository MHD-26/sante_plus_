...
export default function AssistantIAView({ currentUserRole, getHeaders }: AssistantIAViewProps) {
...
Je me suis automatiquement adapté à votre rôle d'activité en tant que **${currentUserRole}** ! Saisissez votre question ou utilisez l'un des modèles de tâches cliniques ci-dessous pour démarrer.`,
  ...
  const getPromptHelpers = (role: UserRole) => {
    switch (role) {
      case UserRole.MEDECIN:
        return [
          {
            label: "Orienter un diagnostic",
            prompt:
              "Donner des orientations diagnostiques et une liste de diagnostics différentiels préliminaires pour un patient d'Abidjan souffrant de fièvre aiguë, céphalées intenses et frissons depuis 3 jours.",
          },
          {
            label: "Canevas Compte-Rendu",
            prompt:
              "Rédiger un modèle vide et propre de compte-rendu d'examen clinique général pour un dossier médical.",
          },
          {
            label: "Conseils Hypertension",
            prompt:
              "Rédiger une fiche de conseils d'hygiène de vie et d'alimentation (régime hyposodé) claire pour un patient ivoirien nouvellement hypertendu.",
          },
        ];
      case UserRole.ACCUEIL:
        return [
          {
            label: "SMS excuses retard",
            prompt:
              "Rédiger un SMS poli et rassurant pour informer les patients d'un retard de 30 minutes de leur médecin consultant.",
          },
          {
            label: "Réponse plainte d'attente",
            prompt:
              "Rédiger une lettre d'excuses officielle chaleureuse en réponse à un patient mécontent du temps d'attente à l'accueil.",
          },
          {
            label: "Script rappel WhatsApp",
            prompt:
              "Rédiger un script de conversation téléphonique d'accueil chaleureux pour confirmer les rendez-vous du lendemain.",
          },
        ];
      case UserRole.COMPTABLE:
        return [
          {
            label: "Relance facture en attente",
            prompt:
              "Rédiger un courriel de relance courtois mais ferme pour un dossier d'assurance santé tiers-payant en retard de paiement.",
          },
          {
            label: "Explication Tiers-Payant",
            prompt:
              "Expliquer de façon simple au personnel d'accueil le mécanisme de fonctionnement et d'encaissement d'une facture sous le régime du Tiers-Payant d'assurance.",
          },
        ];
      case UserRole.PHARMACIE:
        return [
          {
            label: "Fiche conseils Coartem",
            prompt:
              "Rédiger une notice explicative simplifiée et bienveillante pour expliquer la prise du traitement antipaludéen Coartem (Artéméther/Luméfantrine) à un patient âgé.",
          },
          {
            label: "Conseils conservation sirops",
            prompt:
              "Rédiger une liste de conseils pratiques à placarder à la pharmacie concernant les règles de conservation des antibiotiques pédiatriques reconstitués.",
          },
        ];
      default:
        return [
          {
            label: "Conseils de santé publique",
            prompt:
              "Rédiger un court dépliant de sensibilisation sur la prévention du paludisme et de la dengue en saison des pluies en Côte d'Ivoire.",
          },
          {
            label: "Formuler un e-mail",
            prompt:
              "Rédiger un e-mail professionnel interne pour inviter tout le personnel de la clinique à la réunion de transformation numérique.",
          },
        ];
    }
  };
  ...
  const getSystemInstruction = (role: UserRole) => {
    const base =
      "Vous êtes l'assistant IA de la Clinique Santé Plus en Côte d'Ivoire. Soyez rigoureux, poli, écrivez dans un français soigné et compréhensible pour des non-techniques. Adoptez un ton professionnel et clinique.";
    ...
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 animate-fade-in max-w-7xl mx-auto h-[calc(100vh-200px)] min-h-[500px]">
      ...
      <div className="bg-white border border-slate-100 rounded-xl p-5 shadow-xs space-y-4 lg:col-span-1 flex flex-col justify-between overflow-y-auto">
        <div className="space-y-4">
          <div className="border-b border-slate-100 pb-3">
            ...
            <p className="text-[10px] text-slate-500 mt-1">
              Modèles de prompts cliniques pré-paramétrés pour votre rôle de{" "}
        ...
        <div className="bg-slate-50 border border-slate-100 p-3 rounded-lg text-[10px] text-slate-500 leading-relaxed space-y-2 mt-4">
          ...
          <p>
            Toutes les réponses de l'IA sont fournies à titre indicatif d'aide à la décision. L'avis
            final de santé d'un praticien agréé de la clinique reste requis.
          </p>
        </div>
      </div>
          ...
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-emerald-700 font-medium">
                Spécialiste de Soutien Clinique Adapté • {currentUserRole}
              </p>
            </div>
          </div>
          ...
          {messages.map((msg, idx) => (
            ...
            >
              ...
              <div className={`space-y-1 max-w-[80%] ${msg.sender === "user" ? "text-right" : ""}`}>
                <div
                  className={`text-xs p-3 rounded-xl inline-block leading-relaxed font-sans text-left shadow-xs ${
                    msg.sender === "user"
                ...
                <span className="block text-[8px] text-slate-400 font-mono">{msg.timestamp}</span>
              </div>
            </div>
          ))}
