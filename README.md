...
_Note : Le fichier `.env` est déjà listé dans `.gitignore` et ne sera jamais envoyé sur GitHub._
...
2.  **Vérifier le statut des fichiers** (s'assurer que le fichier `.env` est bien exclu) :
    ...
    _Vérifiez qu'aucun fichier `.env` n'apparaît dans la liste des fichiers à indexer._
